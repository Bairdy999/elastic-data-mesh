...
function getElasticTransport(uriPath) {
  // From v8 JS client, URL validation fails if there is a URI path, e.g. https://example.com/elastic no longer works (see https://github.com/elastic/elasticsearch-js/issues/1709 for more info)
  // We therefore need to create a class that extends the Transport class to modify the path at runtime:
  return class MTransport extends Transport {
    request(params, options, callback) {
      params.path = uriPath + params.path // <- append the path right here
...
(async () => {
  try {
    ...
    let meshClusterSize = process.env.ELASTIC_MESH_CLUSTER_SIZE;
    let dataType = process.env.POLICE_DATA_TYPE;
    let startTime = new Date();

    let elsMeshBase = process.env.ELASTIC_MESH_ENDPOINT;
    let elsMeshUser = process.env.ELASTIC_MESH_USER;
    let elsMeshPassword = process.env.ELASTIC_MESH_PASSWORD;
    let elsMeshLoad = process.env.ELASTIC_MESH_LOAD;


    let elsClient;

    if (!elsMeshLoad) {
      elsClient = new Client({
        node: elsEndpoint,
    ...
    for (let x = dataStartYear; x <= dataEndYear; x++) {
      ...
      for (let y = 1; y <= 12; y++) {
        ...
        for (let z = 0; z < fileLoopSize; z++) {
          ...
          if (elsMeshLoad) {
            let clusterNum = (z + 1).toString().padStart(2, '0');
//            let meshEndpoint = elsMeshBase + "/cluster" + clusterNum + "/elastic/";
            let meshEndpoint = elsMeshBase;
            let meshUriPath = "/cluster" + clusterNum + "/elastic";
//            console.log(meshEndpoint);
            elsClient = new Client({
              node: meshEndpoint,
              Transport: getElasticTransport(meshUriPath),
              headers: { 'custom': 'Accept:application/vnd.elasticsearch+json;compatible-with=8' },
              auth: {
                username: elsMeshUser,
                password: elsMeshPassword
              },
              tls: {
                rejectUnauthorized: false
              }
