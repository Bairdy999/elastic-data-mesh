...
#### Items of note
- As Docker Compose doesn't support dynamic runtime variables, e.g. in a for loop for the cluster instance number, (standard environment variables themselves aren't dynamic variables int the same context), the Docker Compose YAML file is treated as a template, passed to the `[envsubst](https://manpages.ubuntu.com/manpages/noble/man1/envsubst.1.html)` process to inject the cluster instance number and pass the resultant YAML to Docker Compose via stdin
