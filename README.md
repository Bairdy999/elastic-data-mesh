...
- As Docker Compose doesn't support dynamic runtime variables, e.g. in a `for` loop for the cluster instance number (standard environment variables themselves aren't dynamic variables in the same context), the Docker Compose YAML file is treated as a template, passed to the [envsubst](https://manpages.ubuntu.com/manpages/noble/man1/envsubst.1.html) process to inject the cluster instance number, and then the resultant YAML piped to Docker Compose via stdin. As such it cannot currently be used directly with Docker Compose.
